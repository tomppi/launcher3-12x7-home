 ...
 *
 * <p>The module keeps Launcher3/Quickstep as the stock Recents provider while changing only the
 * workspace grid, foldable workspace mode, and widget resize limits. All Apps, folders, hotseat,
 * taskbar, gestures, and Recents are deliberately left alone.</p>
 */
public final class LauncherGridHook implements IXposedHookLoadPackage {
    ...
    private static final String FEATURE_FLAGS_CLASS = "com.android.launcher3.config.FeatureFlags";
    private static final String BOOLEAN_FLAG_CLASS =
    ...
    public void handleLoadPackage(XC_LoadPackage.LoadPackageParam lpparam) {
        ...
        enableFoldableSinglePage(lpparam.classLoader);
    ...
    /**
     * Replaces only FeatureFlags.FOLDABLE_SINGLE_PAGE with the ENABLED enum value.
     *
     * <p>It is important not to modify BooleanFlag.DISABLED itself, because many unrelated Launcher3
     * flags share that same enum singleton.</p>
     */
    private static void enableFoldableSinglePage(ClassLoader classLoader) {
        try {
            Class<?> featureFlagsClass = XposedHelpers.findClass(FEATURE_FLAGS_CLASS, classLoader);
            Class<?> booleanFlagClass = XposedHelpers.findClass(BOOLEAN_FLAG_CLASS, classLoader);
            Object enabledFlag = XposedHelpers.getStaticObjectField(booleanFlagClass, "ENABLED");

            XposedHelpers.setStaticObjectField(
                    featureFlagsClass,
                    "FOLDABLE_SINGLE_PAGE",
                    enabledFlag);

            XposedBridge.log(TAG + ": enabled foldable single-page workspace");
        } catch (Throwable error) {
            XposedBridge.log(TAG + ": could not enable foldable single-page workspace");
                ...
                new XC_MethodHook() {
                    ...
                    protected void beforeHookedMethod(MethodHookParam param) {
                        try {
                            ...
                            // Change only the workspace grid. All Apps uses separate fields.
     ...
     * This also makes both horizontal and vertical resize directions available.
     */
